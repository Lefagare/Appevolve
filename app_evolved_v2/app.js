...
// === PATCH: Quick Access navigation + dynamic totalSections ===
(function(){
  function navigateTo(sectionId){
    try {
      ...
      if (window.AppState){
        ...
        if (sectionId !== 'intro' && typeof window.AppState.markSectionComplete === 'function'){
          window.AppState.markSectionComplete(sectionId);
        }
        if (typeof window.AppState.updateProgress === 'function') window.AppState.updateProgress();
        if (typeof window.AppState.save === 'function') window.AppState.save();
      }
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch(e) { console.warn('navigateTo error', e); }
  }

  document.addEventListener('DOMContentLoaded', function(){
    // Make Quick Access links work even without sidebar entries
    ...
    // Update totalSections based on actual number of sections
    try {
      var sections = document.querySelectorAll('main .section');
      if (window.AppState && sections && sections.length){
        var computedTotal = Math.max(0, sections.length - 1); // minus intro
        if (window.AppState.totalSections !== computedTotal){
          window.AppState.totalSections = computedTotal;
          if (typeof window.AppState.updateProgress === 'function') window.AppState.updateProgress();
          if (typeof window.AppState.save === 'function') window.AppState.save();
        }
      }
    } catch(e){ console.warn('totalSections patch failed', e); }
  });
})();
// === END PATCH ===
